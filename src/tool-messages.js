...
Object.assign(TOOL_MESSAGES,{"intent.pixel-lab.title":["픽셀 랩","Pixel Lab","ピクセルラボ"],"intent.pixel-lab.headline":["프레임 전체가 같은 색으로.","One palette. Every frame.","すべてのフレームを同じ色で。"],"intent.pixel-lab.description":["여러 프레임을 한 팔레트로 고정하고 정리·검사한 뒤 내보내세요.","Lock many frames to one palette, clean them up, audit the colour budget and export.","複数フレームを1つのパレットに固定し、整理・検査して書き出します。"],"intent.pixel-lab.action":["픽셀 에셋 만들기","Prepare pixel assets","ドット素材を整える"],"intent.palette-extractor.title":["팔레트 추출","Palette Extractor","パレット抽出"],"intent.palette-extractor.headline":["이 그림의 색, 그대로.","The colours that are really there.","この絵の色、そのまま。"],"intent.palette-extractor.description":["이미지 여러 장에서 팔레트를 한 번에 뽑아 .gpl·HEX·JSON으로 저장하세요.","Extract one palette from several images and save it as .gpl, HEX or JSON.","複数の画像から1つのパレットを抽出し、.gpl・HEX・JSONで保存。"],"intent.palette-extractor.action":["팔레트 추출하기","Extract palette","パレットを抽出"],"intent.palette-swap-ramp.title":["램프 색 교체","Ramp Palette Swap","ランプ色替え"],"intent.palette-swap-ramp.headline":["음영은 그대로, 색만 바꾸기.","Same shading, new colour.","陰影はそのまま、色だけ変える。"],"intent.palette-swap-ramp.description":["음영 램프를 명도 순서대로 대응시키고 팀 컬러 변형을 한 번에 만드세요.","Map a ramp by lightness order and build team-colour variants in one go.","明度順にランプを対応させ、チームカラーを一度に作成。"],"intent.palette-swap-ramp.action":["색 바꾸기","Swap the ramp","色を替える"],"intent.pixel-art-cleanup.title":["픽셀 정리","Pixel Art Cleanup","ドット絵の整理"],"intent.pixel-art-cleanup.headline":["필요 없는 픽셀만 지우기.","Only the pixels that should not be there.","余分なピクセルだけ消す。"],"intent.pixel-art-cleanup.description":["단독 픽셀·작은 덩어리·1픽셀 구멍·안티에일리어싱 잔여물을 찾아 정리하세요.","Find stray pixels, tiny clusters, single-pixel holes and anti-alias leftovers, then clear them.","単独ピクセル・小さな塊・1ピクセルの穴・アンチエイリアスの残りを整理。"],"intent.pixel-art-cleanup.action":["픽셀 정리하기","Clean up pixels","ドットを整理"],"intent.pixel-perfect-checker.title":["픽셀 격자 검사","Pixel Perfect Checker","ピクセル格子の検査"],"intent.pixel-perfect-checker.headline":["진짜 도트 크기를 확인하세요.","How big is one pixel, really?","本当のドットサイズを確認。"],"intent.pixel-perfect-checker.description":["확대된 스프라이트의 도트 크기·격자 어긋남·흐린 경계·색 수를 측정합니다.","Measure the pixel size, grid offset, blurred edges and colour count of an upscaled sprite.","拡大されたスプライトのドットサイズ・格子ずれ・ぼけた境界・色数を測定。"],"intent.pixel-perfect-checker.action":["픽셀 격자 검사하기","Check the pixel grid","格子を検査"]});
