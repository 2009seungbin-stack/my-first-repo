...
export async function removeBackground(source,{signal,progress=()=>{},backend='auto',refine=true,cleanup=false}={}){
 ...
 try{
  ...
  worker??=new Worker(new URL('./matte-worker.js',import.meta.url),{type:'module'});const bitmap=await createImageBitmap(source);if(signal?.aborted){bitmap.close();abort(signal);}
  const result=await new Promise((resolve,reject)=>{
   ...
   try{worker.postMessage({bitmap,backend,refine,cleanup},[bitmap]);}catch(e){done();bitmap.close();reject(e);}
  });
