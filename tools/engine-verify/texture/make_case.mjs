...
import {bundleFiles,lightTexturesFor} from '../../../src/game/normals/export.js';
...
const checks=[];
for(const i of pick){
 const r=frames[i]?.rect||{x:0,y:0,w,h},lit=renderLit(crop(data,r),crop(normal,r),r.w,r.h,scene);
 ...
 writeFileSync(join(args.out,`expected_flipped_${i}.png`),await bytes(await encodeRGBAPNG(renderLit(crop(data,r),crop(normal,r),r.w,r.h,scene,{flipGreen:true}),r.w,r.h)));
