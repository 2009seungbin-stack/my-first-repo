...
export function seoLinks(path,locale,siteURL){
 ...
 const canonical=locale===null&&!path?href(null):href(locale||'en');
 return `<link data-site-seo rel="canonical" href="${canonical}"><meta data-site-seo property="og:url" content="${canonical}">`+LOCALES.map(l=>`<link data-site-seo rel="alternate" hreflang="${l}" href="${href(l)}">`).join('')+`<link data-site-seo rel="alternate" hreflang="x-default" href="${href(X_DEFAULT_LOCALE(path))}">`;
}
/** `path` selects a landing page (src/landings.js); its own title, copy and canonical apply. */
export function structuredData(id,locale,siteURL,path='',neutral=false){
 if(id==='home'&&!path)return homeStructuredData(locale,siteURL,neutral);
 const land=landingText(path,locale);
 const app={'@context':'https://schema.org','@type':'WebApplication',name:`${land?.title||t(`intent.${id}.title`,{},locale)} · ${BRAND.name}`,description:land?.description||t(`intent.${id}.description`,{},locale),applicationCategory:APP_CATEGORY[familyOf(id)]||'MultimediaApplication',operatingSystem:'Web',browserRequirements:'A current web browser with JavaScript',inLanguage:locale,featureList:toolFeatures(id,locale),publisher:{'@type':'Organization',name:BRAND.name}};
...
/** schema.org / Google applicationCategory per tool family (game tools are developer tools; a
 * GameApplication is a game). */
const APP_CATEGORY={game:'DeveloperApplication',image:'MultimediaApplication',video:'MultimediaApplication',pdf:'BusinessApplication'};
const FAMILY=new Map(DIRECTORY.flatMap(([c,ids])=>ids.map(id=>[id,c])));
const familyOf=id=>FAMILY.get(id)||(INTENTS[id]?.editor==='pdf'?'pdf':INTENTS[id]?.editor==='media'?'video':'image');
/** What the tool does, in the visitor's language: the "What this tool does" copy the page shows
 * (src/content.js guide), never internal engine or maturity labels. */
function toolFeatures(id,locale){
 const g=guide(id,locale);
 return String(g?.[1]||t(`intent.${id}.description`,{},locale)).split(/(?<=[.。])\s*/).map(x=>x.trim()).filter(x=>x.length>2);
}
...
function homeStructuredData(locale,siteURL,neutral=false){
 ...
 nodes.push(app);
 return nodes.map(n=>`<script data-site-seo type="application/ld+json">${JSON.stringify(n).replaceAll('<','\\u003c')}</script>`).join('');
}
export function socialMetadata(id,locale,siteURL,overrides={}){
 ...
 return `<meta data-site-seo property="og:type" content="website"><meta data-site-seo property="og:site_name" content="${esc(BRAND.name)}"><meta data-site-seo name="application-name" content="${esc(BRAND.name)}"><meta data-site-seo property="og:locale" content="${{en:'en_US',ko:'ko_KR',ja:'ja_JP'}[locale]}"><meta data-site-seo name="twitter:card" content="summary_large_image"><meta data-site-seo name="twitter:title" content="${esc(title)}"><meta data-site-seo name="twitter:description" content="${esc(description)}">`+(image?`<meta data-site-seo property="og:image" content="${esc(image)}"><meta data-site-seo property="og:image:width" content="1200"><meta data-site-seo property="og:image:height" content="630"><meta data-site-seo property="og:image:alt" content="${esc(title)}"><meta data-site-seo name="twitter:image" content="${esc(image)}">`:'');
}
...
export function updateSEO(id,locale,siteURL,path=''){
 ...
 // The home page served at / (no language segment) keeps / as its canonical, as in the static HTML.
 const neutral=id==='home'&&!path&&!!siteURL&&!/^(ko|en|ja)(\/|$)/.test(location.pathname.slice(new URL(siteURL).pathname.length));
 document.head.insertAdjacentHTML('beforeend',(!mayPromote(id)?'<meta data-quality-robots name="robots" content="noindex,follow">':'')+seoLinks(land?path:INTENTS[id].path,neutral?null:locale,siteURL)+structuredData(id,locale,siteURL,path,neutral)+socialMetadata(id,locale,siteURL,social)+navigationData(id,locale,siteURL,path));
