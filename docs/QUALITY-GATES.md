...
- AI SR: actual 8K ML export, tile-seam/alpha goldens across photo/anime/text/pixel fixtures, efficient fallback. WebGPU inference now succeeds on desktop Chrome/Edge with WASM-identical output; mobile GPUs are unverified. The WASM fallback runs multi-threaded (8 threads measured) in Chromium browsers through the isolated `/ai-runtime/` frame (Document-Isolation-Policy); site-wide COOP/COEP was deliberately not adopted because it would break AdSense and third-party embeds. Firefox and WebKit ignore Document-Isolation-Policy and stay single-threaded (~3× slower on the 2× photo); Chromium versions without DIP likewise. Swin2SR WASM timings (~42 s for the 2× portrait with threads) remain too slow for a Flagship claim, and the isolated frame has not yet been exercised on a deployed Cloudflare origin or on mobile Chrome.
