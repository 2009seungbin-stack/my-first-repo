...
export function mount({el,def}){
 ...
 let o={layout:'packed',padding:2,trim:true,extrude:0,pot:false,rotate:false,maxSize:4096,columns:0,format:'json-hash',name:'',fps:12,outlines:true,pixelated:true,shared:true};
...
<button type="button" class="primary big" id="atlasRun" data-action="atlas-run"></button><nav class="next" id="atlasNext"></nav><small class="local-note">${esc(text('local'))}</small></aside></div>`;
 ...
 function build(){
  ...
  try{
   ...
   for(const f of frames){
    ...
    Im.release(tile);
    const g=o.shared?groups.find(x=>x.canvas&&x.frames.includes(f)):null,at=g?g.canvas.offsets[g.frames.indexOf(f)]:{x:0,y:0};
    data.push({name:f.name,x:p.x+e,y:p.y+e,w:r.w,h:r.h,rotated:p.rotated,trimmed:o.trim&&(r.w!==f.w||r.h!==f.h)||!!g,sourceW:g?g.canvas.w:f.w,sourceH:g?g.canvas.h:f.h,offsetX:r.x+at.x,offsetY:r.y+at.y});
   }
 ...
 async function run(){
  ...
  try{const png=await Im.blobOf(el.querySelector('#atlasCanvas')),dataText=atlasData(o.format,built.data,{image:base+'.png',width:built.width,height:built.height,animations:built.animations});
