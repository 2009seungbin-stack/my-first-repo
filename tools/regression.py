...
env=os.environ.copy()
for key in ['ADSENSE_CLIENT','ADSENSE_VERIFICATION_CLIENT','ADSENSE_SLOT_CONTENT_1','ADSENSE_SLOT_CONTENT_2','ADSENSE_SLOT_STUDIO','ADSENSE_CMP_READY','GOOGLE_SITE_VERIFICATION','INDEXNOW_KEY','TICKET_PUBLIC_KEY','TICKET_PRIVATE_KEY','CF_PAGES','CF_PAGES_BRANCH','SITE_ENV','PORT','BASE_PATH','DIST_DIR']:env.pop(key,None)
...
try:
    ...
    if os.environ.get('REDTEAM')=='1':
        run('redteam-api',['node','tests/redteam/api-attacks.mjs'])
        run('redteam-browser',[sys.executable,'tests/redteam/browser-attacks.py'])
finally:
