...
self.onmessage=async({data:m})=>{try{let result;if(m.kind==='pixel'){result=quantize(m.data,m.w,m.h,m.colors,m.dither);if(m.outline)result=addOutline(result,m.w,m.h,m.outline);}else if(m.kind==='remove')result=removeConnected(m.data,m.w,m.h,m.color,m.tolerance);else if(m.kind==='outline')result=addOutline(m.data,m.w,m.h,m.radius);else if(m.kind==='gif')result=gif(m.frames,m.w,m.h,m.delay);else if(m.kind==='portrait'){
 ...
 const model=await AutoModel.from_pretrained('Xenova/modnet',{dtype:'fp32',device:'wasm',progress_callback:p=>{if(p.status==='progress')self.postMessage({progress:`모델 다운로드 ${Math.round(p.progress||0)}%`});}}),processor=await AutoProcessor.from_pretrained('Xenova/modnet');
 const image=new RawImage(m.data,m.w,m.h,4);self.postMessage({progress:'기기에서 인물과 배경을 분리하는 중…'});
 ...
 }else throw Error('알 수 없는 작업입니다.');self.postMessage({result},[result.buffer]);}catch(e){self.postMessage({error:e.message||String(e)});}};
