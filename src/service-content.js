...
export const S={
 en:{
  ...
  freeFeatures:['The whole Studio and every tool','Full output quality — no watermark, no reduced resolution','Local processing: your files stay on your device','{s} Studio engine exports per day — editing, saving projects and single PNGs are unlimited','{n} heavy jobs per day in the file tools','Ads (in the Studio: one labelled column on large screens)'],
 ...
 ko:{
  ...
  freeFeatures:['스튜디오 전체와 모든 도구','원본 품질 그대로 — 워터마크·해상도 제한 없음','로컬 처리: 파일이 기기 밖으로 나가지 않음','스튜디오 엔진 내보내기 하루 {s}회 — 편집·프로젝트 저장·PNG 한 장 내보내기는 무제한','파일 도구의 무거운 작업 하루 {n}회','광고 표시 (스튜디오에서는 큰 화면에서만 표시된 광고 칸 하나)'],
 ...
 ja:{
  ...
  freeFeatures:['スタジオ全体とすべてのツール','出力品質はそのまま — 透かし・解像度制限なし','ローカル処理：ファイルは端末の外に出ません','スタジオのエンジン書き出しは1日{s}回 — 編集・プロジェクト保存・PNG 1枚の書き出しは無制限','ファイルツールの重い処理は1日{n}回','広告あり（スタジオでは大きな画面でのみ、表示付きの広告枠が1つ）'],
...
export function pricingHTML(locale,{pricing,freeDailyJobs,freeDailyStudio=10}){
 const t=k=>text(locale,k),s=S[locale]||S.en;
 return `<h1>${esc(t('pricingTitle'))}</h1><p class="service-lead">${esc(t('pricingLead'))}</p>
<div class="plans">
<section class="plan" data-plan="free"><h2>${esc(t('freeName'))}</h2><p class="plan-price">${esc(t('freePrice'))}</p>${list(s.freeFeatures.map(f=>f.replace('{n}',freeDailyJobs).replace('{s}',freeDailyStudio)))}<a class="plan-cta secondary" href="${locale}/game/studio/">${esc(t('startFree'))}</a></section>
