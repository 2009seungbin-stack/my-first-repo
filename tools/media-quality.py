...
for row in report['rows']:
 ...
 if not row['audio']:
  frames=json.loads(subprocess.check_output(['ffprobe','-v','error','-select_streams','v','-count_frames','-show_entries','stream=nb_read_frames','-of','json',str(artifact)]));n=int(frames['streams'][0]['nb_read_frames']);assert n==row.get('decodedFramesExpected',60),(row['case'],n)
 ...
 # Normalised audio is deliberately louder than the source; every other case must keep its level.
 low,high=(.55,.75) if row['case']=='normalized' else (.065,.11)
 assert low<rms<high,(row['case'],'440 Hz source level not retained',rms)
 ...
 if row['case']=='faded':
  head=values[:14400];middle=values[len(values)//2-14400:len(values)//2+14400]
  result['fadeInRMS']=math.sqrt(sum(v*v for v in head)/len(head));result['middleRMS']=math.sqrt(sum(v*v for v in middle)/len(middle))
  assert result['fadeInRMS']<result['middleRMS']*.45,result
 if row['case']=='precise':
