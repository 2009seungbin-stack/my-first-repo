...
export async function packMasks(items,w,h,mapping,{signal,progress=()=>{},invert=[]}={}){
 ...
 try{
  try{worker=new Worker(new URL('./mask-worker.js',import.meta.url),{type:'module'});await request({start:true,w,h,mapping,invert});}
  catch(e){abort(signal);worker?.terminate();worker=null;output=maskBuffer(w,h,mapping,invert);}
  for(const index of [...new Set(mapping.filter(Number.isInteger))]){
   ...
   try{
    ...
    for(let y=0;y<h;y+=32){abort(signal);const rows=Math.min(32,h-y),data=ctx.getImageData(0,y,w,rows).data;
     if(worker)await request({buffer:data.buffer,offset:y*w*4,channels},[data.buffer]);else applyMaskRows(output,data,y*w*4,channels,invert);
...
/** `invert` is indexed by channel, so an inverted constant flips (255 → 0) as well. */
export function maskBuffer(w,h,mapping,invert=[]){const output=new Uint8ClampedArray(w*h*4);for(let c=0;c<4;c++){const m=mapping[c];if(m!=='zero'&&m!=='one')continue;if((m==='one')!==!!invert[c])for(let i=c;i<output.length;i+=4)output[i]=255;}return output;}
export function applyMaskRows(output,rgba,offset,channels,invert=[]){for(let i=0;i<rgba.length;i+=4){const value=Math.round(.2126*rgba[i]+.7152*rgba[i+1]+.0722*rgba[i+2]);for(const c of channels)output[offset+i+c]=invert[c]?255-value:value;}}
