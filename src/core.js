 ...
 * With `transparent`, palette index 0 becomes the transparent entry and index 1 carries black, so
 * sprite frames keep their cut-out shape and near-black pixels still read as black. */
export function gif(frames,w,h,delay=100,{transparent=false}={}){const a=[],word=n=>a.push(n&255,n>>8),str=s=>a.push(...new TextEncoder().encode(s));str('GIF89a');word(w);word(h);a.push(0xf7,0,0);for(let i=0;i<256;i++){if(transparent&&i===1)a.push(0,0,0);else a.push(Math.round((i>>5)*255/7),Math.round(((i>>2)&7)*255/7),Math.round((i&3)*255/3));}a.push(0x21,0xff,11);str('NETSCAPE2.0');a.push(3,1,0,0,0);for(const frame of frames){a.push(0x21,0xf9,4,transparent?9:0);word(Math.round(delay/10));a.push(0,0,0x2c);word(0);word(0);word(w);word(h);a.push(0,8);const data=[];let bits=0,val=0;const code=n=>{val|=n<<bits;bits+=9;while(bits>=8){data.push(val&255);val>>>=8;bits-=8;}};const pick=j=>{const index=(frame[j]>>5)<<5|(frame[j+1]>>5)<<2|(frame[j+2]>>6);return transparent?(frame[j+3]<128?0:index||1):index;};for(let start=0;start<w*h;start+=200){code(256);for(let i=start;i<Math.min(start+200,w*h);i++)code(pick(i*4));}code(257);if(bits)data.push(val&255);for(let i=0;i<data.length;i+=255){const block=data.slice(i,i+255);a.push(block.length,...block);}a.push(0);}a.push(0x3b);return new Uint8Array(a);}
