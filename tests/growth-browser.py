...
with sync_playwright() as pw:
    ...
    page.close()
    page=mount(context,'/en/image/editor/?explore=1');expect(page.locator('#toolsDialog')).to_be_visible();page.locator('#toolSearch').fill('remove bg')
    ok('tool search recognizes common aliases',page.locator('[data-action="kit-open:remove-bg"]').count()==1)
    click(page,'kit-close');ok('catalog closes',not page.locator('#toolsDialog').is_visible())
