...
for(const folder of ['src','src/task','src/game','src/game/exporters','src/game/pack','src/game/export','src/studio','src/studio/canvas','src/studio/core','src/studio/ui','src/studio/workspaces','src/studio/sprite','src/studio/pixel','src/studio/workspaces/pixel','server','tools'])for(const file of modules(folder).filter(f=>/\.(mjs|js)$/.test(f))){const r=spawnSync(process.execPath,['--check',`${folder}/${file}`],{stdio:'inherit'});if(r.status)process.exit(r.status);}
