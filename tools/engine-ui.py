...
try:
 ...
 with sync_playwright() as pw:
  ...
  page.goto('http://127.0.0.1:4283/en/video/trim/');page.locator('#fileInput').set_input_files(str(out/'signal-1080p.mp4'));page.locator('#workspace[aria-busy="false"]').wait_for()
  page.locator('#mediaFormat').select_option('mp4');page.locator('#mediaMode').select_option('fast');page.locator('#mediaStart').fill('1');page.locator('#mediaEnd').fill('4');page.locator('[data-action="intent-run"]').click();page.locator('#workspace[aria-busy="false"]').wait_for()
