...
export function updateSiteContent(id,locale,path=''){
 const host=document.getElementById('siteContent');if(!host)return;
 const key=`${id}:${locale}:${path}`;if(host.dataset.rendered===key)return;
 const next=document.createElement('template');next.innerHTML=toolContent(id,locale,path);
