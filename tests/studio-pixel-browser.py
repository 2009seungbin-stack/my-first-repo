...
print(table.concat(out,","))''')
    ...
    for lang,menu,tool,layer in [('ko','픽셀','연필','레이어 1'),('ja','ピクセル','鉛筆','レイヤー 1')]:
        ...
        c2.close()
    # ---------------------------------------- palette count of a new multi-frame import
    ...
    q.set_input_files('input[type=file][multiple]',[str(FIX/f'ninja_run_{i}.png') for i in range(6)]);q.wait_for_function('()=>window.nerulioStudio.doc.assets.length>0');settle(q,1500)
    want=js(q,'const d=W.session.compose(null,{onion:false}),s=new Set();for(let i=0;i<d.length;i+=4)if(d[i+3])s.add(d[i]+","+d[i+1]+","+d[i+2]+","+d[i+3]);return s.size;')
    info=q.inner_text('[data-px="pal-info"]');n=q.locator('.px-pal-sw').count()
    ok('frames imported without a palette: the palette panel lists the colours of the picture (not "0 colours")',want>0 and n==want and info.startswith(f'{want} '),f'want {want}, swatches {n}, info {info!r}')
    c4.close()
    # ------------------------------------------------------------ 390 px
    ...
    # the picture's box (CSS px) against the visible canvas; dx/dy = centre offsets, inside = fully on screen
    PLACE='const v=S.view,r=v.stage.getBoundingClientRect(),k=r.width/v.W,img=v.image,x0=r.left+v.view.x*k,y0=r.top+v.view.y*k,w=img.w*v.view.scale*k,h=img.h*v.view.scale*k;return {dx:Math.round(x0+w/2-(r.left+r.width/2)),dy:Math.round(y0+h/2-(r.top+r.height/2)),inside:x0>=r.left-1&&y0>=r.top-1&&x0+w<=r.right+1&&y0+h<=r.bottom+1,w:Math.round(w),stage:[Math.round(r.width),Math.round(r.height)]};'
    pl=js(q,PLACE)
    ok('390 px: after an import the picture is fitted and centred in the canvas (within 3 px)',abs(pl['dx'])<=3 and abs(pl['dy'])<=3 and pl['inside'] and pl['w']>=200,str(pl))
    shot(q,'06-390.png')
    more=q.eval_on_selector('.st-toolbar','e=>e.dataset.more')
    q.eval_on_selector('.st-toolbar','e=>{e.scrollLeft=e.scrollWidth}');settle(q,200);more2=q.eval_on_selector('.st-toolbar','e=>e.dataset.more')
    q.eval_on_selector('.st-toolbar','e=>{e.scrollLeft=0}');settle(q,200)
    ok('390 px: the tool strip fades the edge that hides more tools (end, then start after scrolling)',more=='end' and more2=='start' and q.eval_on_selector('.st-toolbar','e=>getComputedStyle(e).maskImage||getComputedStyle(e).webkitMaskImage')!='none',f'{more} {more2}')
    q.click('.st-compact-only:not(.st-hamburger)');settle(q,700)
    ...
    pl2=js(q,PLACE)
    ok('390 px: when the sheet shrinks the canvas, the fitted picture is fitted and centred again',abs(pl2['dx'])<=3 and abs(pl2['dy'])<=3 and pl2['inside'] and pl2['stage'][1]<pl['stage'][1],str(pl2))
    shot(q,'07-390-panels.png')
    q.click('.st-compact-only:not(.st-hamburger)');settle(q,700)
    pl3=js(q,PLACE);ok('390 px: closing the sheet fits and centres it again',abs(pl3['dx'])<=3 and abs(pl3['dy'])<=3 and pl3['inside'],str(pl3))
