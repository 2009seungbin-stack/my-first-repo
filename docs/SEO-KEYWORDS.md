...
pages themselves are in `src/game-seo-{broad,engines,formats,fixes,compare}.js` (see
...
|---|---|
| pixel art editor (online, web, browser), ドット絵 エディタ ブラウザ, 도트 에디터 (AC-G/B) | The Studio does not draw pixels yet (the Pixel workspace is coming). A page would promise a painter we do not have. Revisit when the Pixel workspace ships. |
