...
export function createTools(W){
 ...
 function cancel(){if(!g)return;if(g.stroke){g.stroke.revert();if(g.dirty)W.session.refresh(g.dirty);}g=null;W.overlayDraft(null);}
 ...
 const freehand=(tool,label)=>({
  cursor:()=>'crosshair',get active(){return !!g;},
  down(info){if(info.button!==0&&info.button!==2)return false;if(tool!=='eraser'&&altPick(info))return false;
   ...
   touch();},
  move(info){if(!g||g.line)return;const [x,y]=px(info);g.stroke.point(x,y);g.last=[x,y];touch();},
  up(){return finish(W.t(label));},
  cancel,hover(info){W.brushHover(px(info));},leave(){W.brushHover(null);}
 });
 // ------------------------------------------------------------------ shapes: line, rectangle, ellipse
 const shapeTool=(tool,label)=>({
  cursor:()=>'crosshair',get active(){return !!g;},
  down(info){if(info.button!==0&&info.button!==2)return false;if(altPick(info))return false;if(!begin(tool,info,{shape:true}))return false;this.move(info);},
  move(info){if(!g)return;let [x,y]=px(info);const [x0,y0]=g.start;const fill=tool!=='line'&&W.opts.shapeFill;
