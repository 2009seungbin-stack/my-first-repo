...
with sync_playwright() as pw:
    ...
    ok('the check panel reacts at once and its headline names the reason (a combination has no tile)',p.locator('[data-check="missing"]').first.get_attribute('data-missing')!='0' and 'combination' in p.locator('[data-verdict="open"]').inner_text())
    ...
    ok('a new map starts with a painted island (lake, peninsula, islet) drawn correctly',js(p,'return Object.values(S.doc.settings.tile.maps)[0].layers[0].cells.replace(/\\./g,"").length;')>100)
    js(p,'S.view.zoomTo(1);S.view.reveal({x:22*64,y:13*64,w:64,h:64});')
    p.wait_for_timeout(300)
    pt=image_point(p,22*64+32,13*64+32);p.mouse.click(pt['x'],pt['y']);p.wait_for_timeout(500)
    ...
    js(p,'S.view.fit();');p.wait_for_timeout(300);shot(p,'05-map-missing-1440.png')
    ...
    p.wait_for_selector('[data-verdict="generated"],[data-verdict="complete"]',timeout=15000)
    v=p.locator('[data-verdict]').first.inner_text()
    ok('the generated set\'s headline says why the art check does not apply (or that it measured and agreed)',('pixel for pixel' in v) or v.startswith('Complete'),v)
    ...
    p.wait_for_selector('[data-verdict="generated"],[data-verdict="complete"]',timeout=15000);p.wait_for_timeout(300)
