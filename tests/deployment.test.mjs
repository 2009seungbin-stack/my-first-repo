...
test('canonical aliases consolidate and query combinations never enter sitemap',()=>{
 ...
 assert.equal((xml.match(/<url>/g)||[]).length,(Object.keys(INTENTS).filter(mayPromote).length+LANDING_PATHS.filter(p=>mayPromote(LANDINGS[p].intent)).length+POLICY_ROUTES.length)*3);
