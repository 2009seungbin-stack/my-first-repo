...
export const INTENTS=Object.freeze({
 ...
 'remove-bg':spec('image/remove-bg','image','background','background','background','image',['pixel','resize','convert']),
 compress:spec('image/compress','image','export','download','compress','image',['convert','resize','pixel']),
 ...
 pixel:spec('pixel','pixel','pixel','pixel','pixel','image',['image','upscale','convert']),
...
export const ALIASES=Object.freeze({'image/pixel':'pixel','image/target-size':'compress','png-to-webp':'convert','jpg-to-png':'convert','webp-to-jpg':'convert','image/remove-background':'remove-bg'});
...
export function intentDefaults(id,path='',search=''){
 ...
 return {format,kb:number('kb',id==='compress'?500:0,0,32768),width:number('w',0,0,8192),height:number('h',0,0,8192),scale:number('scale',2,2,4)===4?4:2,n:number('n',32,8,512),background:q.get('mode')==='portrait'?'portrait':'solid',mediaFormat:id==='video-mp3'?'mp3':id==='video-gif'?'gif':'webm',pdfFormat:id==='pdf-to-jpg'?'jpeg':'pdf',range:q.get('pages')||''};
}
